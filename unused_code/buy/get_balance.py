...
class OrderService(threading.Thread):
    def __init__(self, is_notification=False, can_open_orders=False):
        ...
        self.telegram_service = TelegramService(
            NOTIFICATION["bot_token"], NOTIFICATION["chat_id"]
        )
    ...
    def create_order(self, order: Order):
        try:
            ...
            if is_order_already_open(self.orders, order):
                logger.info(
                    f"сделка в направлении {order.direction} уже открыта: {order}"
                )
                return
            ...
            if CAN_REVERSE_ORDER:
                ...
                if len(active_orders) > 0:
                    ...
                    logger.info(
                        f"переворачиваю позицию - поступила сделка в обратном направлении: {order}"
                    )
    ...
    def close_order(self, order: Order, close_price: float):
        ...
        if order.is_win:
            logger.info(
                f"закрыта заявка по тейк-профиту с результатом {order.result}; открыта в {order.time}"
            )
        else:
            logger.info(
                f"закрыта заявка по стоп-лоссу с результатом {order.result}; открыта в {order.time}"
            )
