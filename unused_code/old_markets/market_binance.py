        ...
        if dict_message.get("e") == "aggTrade":
            ...
            if formatted_time in self.last_prices.keys():
                ...
                if len(self.last_prices.items()) > 1:
                    ...
                    if prev_data[1]["volume"] > 2 * 10**6:
                        now = datetime.now()
                        price_delta = round(
                            (prev_data[1]["last_price"] - prev_data[1]["first_price"])
                            / (
                                (prev_data[1]["max_price"] + prev_data[1]["min_price"])
                                / 2
                            )
                            * 100,
                            2,
...
Цена: {prev_data[1]['last_price']} $"""
                        ...
                        self._loop.create_task(
                            self._tg_bot.send_signal(message_to_send, "binance")
                        )
