...
from candle_model import Candle, TradeData, candle_type_analysis


def moving_average_and_sko_calc(input_list):
    _summ = 0
    for elem in input_list:
        _summ += elem
    _avarege = _summ / len(input_list)
...
def bollinger_and_rsi_data(figi: str, candle_data: HistoricCandle):
            ...
            if time_now.minute == time_stamp and time_now.hour in working_hours:
                for share in shares:
                    async for candle in client.get_all_candles(
                        ...
                        # )
                        # if (
                        #     (time_now.astimezone(pytz.UTC) - candle.time).seconds
                        #     // 3600
                        # ) == 1:
                        bollinger_and_rsi_data(share["figi"], candle)
                #                             message_to_send = f"""
                # СТРАТЕГИЯ НИКИТА
